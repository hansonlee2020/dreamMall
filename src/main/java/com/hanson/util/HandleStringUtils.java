...
public class  HandleStringUtils {
    ...
    */
    public static String subStringBySymbol(String original){
        String symbol = "[";    //截取标记
        String target = new String();   //目标字符串
    ...
    public static String formatString(String original ,String replace, String replaceValue,Integer index,boolean isValueString) throws NullPointerException{
        ...
        if(index < items.length){
            ...
            if (isValueString){     //替换的值为字符串类型
                tempResult = stringBuffer.append("\"").append(temp[0]).append("\":").append("\"").append(temp[1]).append("\"").toString();
            }else {                 //替换的值为非字符串类型
                tempResult = stringBuffer.append("\"").append(temp[0]).append("\":").append(temp[1]).toString();
            }
            items[index] = tempResult;
            StringBuffer itemsBuffer = new StringBuffer("{");
            for (int i = 0 ; i < items.length ; i ++){
                itemsBuffer.append(items[i]).append(",");
            }
    ...
    */
    public static String increaseString(String original,String name,String value){
        ...
        if(original.length() > 0 ){
            ...
            StringBuffer stringBuffer = new StringBuffer();
            result = (stringBuffer.append("{").append("\"").append(name).append("\":").append("\"").append(value).append("\"").append(",").append(substring)).toString();
            return result;
        }
        return "源字符串不能为空";
    }
    ...
    */
    public static Set<String> formatToSet(String original){
        ...
        if (original.indexOf(",") < 0){//源字符串不含拆分标记直接返回
            result.add(original);
            return result;
        }
